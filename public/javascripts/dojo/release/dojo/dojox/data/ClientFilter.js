		...
		cf = dojo.declare("dojox.data.ClientFilter", null, {cacheByDefault:false, constructor:function () {
			...
			if (this.isUpdateable(request)) {
				for (var i = request._version || 0; i < this._updates.length; i++) {
					...
					if (create && this.matchesQuery(create, request) && dojo.indexOf(resultSet, create) == -1) {
			...
			for (var i in argsSuper.query) {
				if (clientQuery[i] == argsSuper.query[i]) {
					...
					if (!(typeof argsSuper.query[i] == "string" && dojo.data.util.filter.patternToRegExp(argsSuper.query[i]).test(clientQuery[i]))) {
						return false;
					}
				}
			}
			...
			if (!defResult) {
				...
				if (putInCache === undefined ? this.cacheByDefault : putInCache) {
					if (args.start || args.count) {
						...
						args.clientQuery = dojo.mixin(args.clientQuery || {}, {start:args.start, count:args.count});
					}
					args = serverArgs;
			...
			for (var i in query) {
				...
				if ((typeof match == "string" && (match.match(/[\*\.]/) || ignoreCase)) ? !dojo.data.util.filter.patternToRegExp(match, ignoreCase).test(value) : value != match) {
					return false;
				}
			}
			return true;
