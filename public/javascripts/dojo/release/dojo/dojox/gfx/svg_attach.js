...
(function () {
	...
	var _getGradient = function (defaultGradient, gradient) {
		...
		for (var i = 0; i < gradient.childNodes.length; ++i) {
			fillStyle.colors.push({offset:gradient.childNodes[i].getAttribute("offset"), color:new dojo.Color(gradient.childNodes[i].getAttribute("stop-color"))});
		}
		return fillStyle;
	};
	...
	var attachTransform = function (object) {
		...
		if (matrix.match(/^matrix\(.+\)$/)) {
			...
			object.matrix = dojox.gfx.matrix.normalize({xx:parseFloat(t[0]), xy:parseFloat(t[2]), yx:parseFloat(t[1]), yy:parseFloat(t[3]), dx:parseFloat(t[4]), dy:parseFloat(t[5])});
		} else {
			object.matrix = null;
		}
	};
