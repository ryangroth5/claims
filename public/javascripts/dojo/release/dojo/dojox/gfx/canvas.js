...
if (!dojo._hasResource["dojox.gfx.canvas"]) {
	...
	(function () {
		var d = dojo, g = dojox.gfx, gs = g.shape, ga = g.arc, m = g.matrix, mp = m.multiplyPoint, pi = Math.PI, twoPI = 2 * pi, halfPI = pi / 2;
		d.extend(g.Shape, {_render:function (ctx) {
			ctx.save();
		...
		d.forEach(["moveTo", "lineTo", "hLineTo", "vLineTo", "curveTo", "smoothCurveTo", "qCurveTo", "qSmoothCurveTo", "arcTo", "closePath"], function (method) {
		...
		g.createSurface = function (parentNode, width, height) {
			...
			var s = new g.Surface(), p = d.byId(parentNode), c = p.ownerDocument.createElement("canvas");
			c.width = dojox.gfx.normalizedLength(width);
			c.height = dojox.gfx.normalizedLength(height);
			p.appendChild(c);
			s.rawNode = c;
			s._parent = p;
			s.surface = s;
