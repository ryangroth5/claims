...
(function () {
	...
	var attachFill = function (object) {
		...
		if (fo.on && fo.type == "gradient") {
			var fillStyle = dojo.clone(dojox.gfx.defaultLinearGradient), rad = dojox.gfx.matrix._degToRad(fo.angle);
			...
			if (fo.on && fo.type == "gradientradial") {
				var fillStyle = dojo.clone(dojox.gfx.defaultRadialGradient), w = parseFloat(r.style.width), h = parseFloat(r.style.height);
				...
				if (fo.on && fo.type == "tile") {
					...
					fillStyle.width = dojox.gfx.pt2px(fo.size.x);
					fillStyle.height = dojox.gfx.pt2px(fo.size.y);
	...
	var attachStroke = function (object) {
		...
		var strokeStyle = object.strokeStyle = dojo.clone(dojox.gfx.defaultStroke), rs = r.stroke;
	...
	var attachTransform = function (object) {
		...
		object.matrix = dojox.gfx.matrix.normalize({xx:sm.xtox, xy:sm.ytox, yx:sm.xtoy, yy:sm.ytoy, dx:dojox.gfx.pt2px(so.x), dy:dojox.gfx.pt2px(so.y)});
	};
	...
	var attachRect = function (object) {
		...
		object.shape = dojox.gfx.makeParameters(dojox.gfx.defaultRect, {x:parseInt(style.left), y:parseInt(style.top), width:width, height:height, r:Math.min(width, height) * arcsize});
	};
	var attachEllipse = function (object) {
		var style = object.rawNode.style, rx = parseInt(style.width) / 2, ry = parseInt(style.height) / 2;
		object.shape = dojox.gfx.makeParameters(dojox.gfx.defaultEllipse, {cx:parseInt(style.left) + rx, cy:parseInt(style.top) + ry, rx:rx, ry:ry});
	};
	var attachCircle = function (object) {
		var style = object.rawNode.style, r = parseInt(style.width) / 2;
		object.shape = dojox.gfx.makeParameters(dojox.gfx.defaultCircle, {cx:parseInt(style.left) + r, cy:parseInt(style.top) + r, r:r});
	};
	var attachLine = function (object) {
		var shape = object.shape = dojo.clone(dojox.gfx.defaultLine), p = object.rawNode.path.v.match(dojox.gfx.pathVmlRegExp);
		do {
	...
	var attachPolyline = function (object) {
		var shape = object.shape = dojo.clone(dojox.gfx.defaultPolyline), p = object.rawNode.path.v.match(dojox.gfx.pathVmlRegExp);
		do {
			if (p.length < 3 || p[0] != "m") {
				break;
			}
	...
	var attachImageTransform = function (object) {
		...
		object.matrix = dojox.gfx.matrix.normalize({xx:m.M11, xy:m.M12, yx:m.M21, yy:m.M22, dx:m.Dx, dy:m.Dy});
	};
	var attachText = function (object) {
		var shape = object.shape = dojo.clone(dojox.gfx.defaultText), r = object.rawNode, p = r.path.v.match(dojox.gfx.pathVmlRegExp);
		do {
			if (!p || p.length != 7) {
				break;
			}
	...
	var attachFont = function (object) {
		var fontStyle = object.fontStyle = dojo.clone(dojox.gfx.defaultFont), c = object.rawNode.childNodes, i = 0;
		for (; i < c.length && c[i].tagName == "textpath"; ++i) {
		}
		if (i >= c.length) {
	...
	var attachPath = function (object) {
		var shape = object.shape = dojo.clone(dojox.gfx.defaultPath), p = object.rawNode.path.v.match(dojox.gfx.pathVmlRegExp), t = [], skip = false, map = dojox.gfx.Path._pathVmlToSvgMap;
