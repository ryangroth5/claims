...
if (!dojo._hasResource["dojox.charting.widget.Chart2D"]) {
	...
	(function () {
		...
		dojo.declare("dojox.charting.widget.Chart2D", dijit._Widget, {theme:null, margins:null, stroke:null, fill:null, buildRendering:function () {
			...
			var axes = d.query("> .axis", n).map(collectAxisParams).filter(notNull), plots = d.query("> .plot", n).map(collectPlotParams).filter(notNull), actions = d.query("> .action", n).map(collectActionParams).filter(notNull), series = d.query("> .series", n).map(collectDataParams).filter(notNull);
			n.innerHTML = "";
