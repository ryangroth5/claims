...
if (!dojo._hasResource["dojox.editor.plugins.NormalizeIndentOutdent"]) {
	...
	dojo.declare("dojox.editor.plugins.NormalizeIndentOutdent", dijit._editor._Plugin, {indentBy:40, indentUnits:"px", setEditor:function (editor) {
		this.editor = editor;
		...
		if (type) {
			if (prevTag && prevTag.tagName.toLowerCase() == "li") {
				...
				dojo.withGlobal(ed.window, "selectElementChildren", dijit._editor.selection, [listItem]);
				dojo.withGlobal(ed.window, "collapse", dijit._editor.selection, [true]);
			}
		}
