...
if (!dojo._hasResource["dojox.widget.gauge.AnalogArcIndicator"]) {
	...
	dojo.declare("dojox.widget.gauge.AnalogArcIndicator", [dojox.widget.gauge.AnalogLineIndicator], {_createArc:function (val) {
		if (this.shapes[0]) {
		...
		if (this.shapes) {
			...
			this.shapes = [this._gauge.surface.createPath().setStroke(stroke).setFill(this.color)];
			this._createArc(v);
			if (this.hover) {
