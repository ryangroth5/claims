...
if (!dojo._hasResource["dojox.rpc.Service"]) {
	...
	dojo.declare("dojox.rpc.Service", null, {constructor:function (smd, options) {
		...
		return dojo.mixin(request, {sync:dojox.rpc._sync, contentType:contentType, headers:{}, target:request.target || dojox.rpc.getTarget(smd, method), transport:method.transport || smd.transport || request.transport, envelope:method.envelope || smd.envelope || request.envelope, timeout:method.timeout || smd.timeout, callbackParamName:method.callbackParamName || smd.callbackParamName, schema:schema, handleAs:request.handleAs || "auto", preventCache:method.preventCache || smd.preventCache, frameDoc:this._options.frameDoc || undefined});
