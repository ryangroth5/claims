...
if (!dojo._hasResource["dojox.drawing.stencil._Base"]) {
	...
	dojox.drawing.stencil._Base = dojox.drawing.util.oo.declare(function (options) {
		...
		var n = this.style.norm, t = this.style.text, ts = this.textSelected || {}, o, nm, width, styleWas = dojo.toJson(n), textWas = dojo.toJson(t);
		var coords = {x:true, y:true, r:true, height:true, width:true, r:true, radius:true, angle:true};
		...
		if (!this._label) {
			this._label = new dojox.drawing.annotations.Label({text:text, util:this.util, mouse:this.mouse, stencil:this, annotation:true, container:this.container, labelPosition:this.labelPosition});
		} else {
			if (text) {
