...
if (!dojo._hasResource["dojox.dtl.dom"]) {
	...
	(function () {
							...
							if (node.getAttribute) {
								...
								if (key == "href" || key == "src") {
									...
									if (key == "tstyle") {
										...
										if (dd.BOOLS[key.slice(1)] && dojo.trim(value)) {
											key = key.slice(1);
										} else {
											if (this._uppers[key] && dojo.trim(value)) {
		...
		dd.DomBuffer = dojo.extend(function (parent) {
			this._parent = parent;
			this._cache = [];
		...
		dd._DomNodeList = dojo.extend(function (nodes) {
			this.contents = nodes || [];
		}, {push:function (node) {
