...
if (!dojo._hasResource["dojox.image.MagnifierLite"]) {
	...
	dojo.declare("dojox.image.MagnifierLite", dijit._Widget, {glassSize:125, scale:6, postCreate:function () {
		...
		var node = this.glassNode = dojo.create("div", {style:{height:this.glassSize + "px", width:this.glassSize + "px"}, className:"glassNode"}, dojo.body());
		...
		dojo.style(this.glassNode, {top:Math.floor(e.pageY - sub) + "px", left:Math.floor(e.pageX - sub) + "px"});
