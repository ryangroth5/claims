...
if (!dojo._hasResource["dojox.cometd.longPollTransportJsonEncoded"]) {
	...
	dojox.cometd.longPollTransportJsonEncoded = new function () {
		...
		this.tunnelInit = function () {
			var message = {channel:"/meta/connect", clientId:this._cometd.clientId, connectionType:this._connectionType, id:"" + this._cometd.messageId++};
		...
		this._connect = function () {
			...
			if ((this._cometd._advice) && (this._cometd._advice["reconnect"] == "handshake")) {
				...
				if (this._cometd._status == "connected") {
					var message = {channel:"/meta/connect", connectionType:this._connectionType, clientId:this._cometd.clientId, id:"" + this._cometd.messageId++};
		...
		this.openTunnelWith = function (messages, url) {
			...
			var post = {url:(url || this._cometd.url), postData:dojo.toJson(messages), contentType:"text/json;charset=UTF-8", handleAs:this._cometd.handleAs, load:dojo.hitch(this, function (data) {
				...
				this._cometd._polling = false;
				var metaMsg = {failure:true, error:err, advice:this._cometd._advice};
		...
		this.sendMessages = function (messages) {
			...
			return dojo.rawXhrPost({url:this._cometd.url || dojo.config["cometdRoot"], handleAs:this._cometd.handleAs, load:dojo.hitch(this._cometd, "deliver"), postData:dojo.toJson(messages), contentType:"text/json;charset=UTF-8", error:dojo.hitch(this, function (err) {
		...
		this.disconnect = function () {
			var message = {channel:"/meta/disconnect", clientId:this._cometd.clientId, id:"" + this._cometd.messageId++};
			message = this._cometd._extendOut(message);
			dojo.rawXhrPost({url:this._cometd.url || dojo.config["cometdRoot"], handleAs:this._cometd.handleAs, postData:dojo.toJson([message]), contentType:"text/json;charset=UTF-8"});
		};
