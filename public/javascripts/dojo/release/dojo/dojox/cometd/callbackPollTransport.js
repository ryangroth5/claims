...
if (!dojo._hasResource["dojox.cometd.callbackPollTransport"]) {
	...
	dojox.cometd.callbackPollTransport = new function () {
		...
		this.openTunnelWith = function (content, url) {
			...
			var script = {load:dojo.hitch(this, function (data) {
				...
				this.tunnelCollapse();
			}), url:(url || this._cometd.url), content:{message:dojo.toJson(content)}, callbackParamName:"jsonp"};
		...
		this.sendMessages = function (messages) {
			...
			var bindArgs = {url:this._cometd.url || dojo.config["cometdRoot"], load:dojo.hitch(this._cometd, "deliver"), callbackParamName:"jsonp", content:{message:dojo.toJson(messages)}, error:dojo.hitch(this, function (err) {
		...
		this.disconnect = function () {
			...
			message = this._cometd._extendOut(message);
			dojo.io.script.get({url:this._cometd.url || dojo.config["cometdRoot"], callbackParamName:"jsonp", content:{message:dojo.toJson([message])}});
		};
