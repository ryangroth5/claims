...
if (!dojo._hasResource["dojox.cometd.longPollTransportFormEncoded"]) {
	...
	dojox.cometd.longPollTransportFormEncoded = new function () {
		...
		this.openTunnelWith = function (content, url) {
			...
			var post = {url:(url || this._cometd.url), content:content, handleAs:this._cometd.handleAs, load:dojo.hitch(this, function (data) {
		...
		this.sendMessages = function (messages) {
			...
			return dojo.xhrPost({url:this._cometd.url || dojo.config["cometdRoot"], handleAs:this._cometd.handleAs, load:dojo.hitch(this._cometd, "deliver"), content:{message:dojo.toJson(messages)}, error:dojo.hitch(this, function (err) {
		...
		this.disconnect = function () {
			...
			message = this._cometd._extendOut(message);
			dojo.xhrPost({url:this._cometd.url || dojo.config["cometdRoot"], handleAs:this._cometd.handleAs, content:{message:dojo.toJson([message])}});
		};
