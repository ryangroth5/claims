...
if (!dojo._hasResource["dojox.highlight._base"]) {
	...
	(function () {
		var dh = dojox.highlight, C_NUMBER_RE = "\\b(0x[A-Za-z0-9]+|\\d+(\\.\\d+)?)";
		...
		dojo.extend(Highlighter, {buildRes:function () {
		...
		dojox.highlight.init = function (node) {
			...
			var classes = node.className.split(/\s+/), flag = dojo.some(classes, function (className) {
