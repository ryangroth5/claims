...
if (!dojo._hasResource["dojox.layout.GridContainer"]) {
	...
	dojo.declare("dojox.layout.GridContainer", [dijit._Widget, dijit._Templated, dijit._Container, dijit._Contained], {templateString:dojo.cache("dojox.layout", "resources/GridContainer.html", "<div id=\"${id}\" class=\"gridContainer\" dojoAttachPoint=\"containerNode\" tabIndex=\"0\" dojoAttachEvent=\"onkeypress:_selectFocus\">\n\t<table class=\"gridContainerTable\" dojoAttachPoint=\"gridContainerTable\" cellspacing=\"0\" cellpadding=\"0\">\n\t\t<tbody class=\"gridContainerBody\">\n\t\t\t<tr class=\"gridContainerRow\" dojoAttachPoint=\"gridNode\"></tr>\n\t\t</tbody>\n\t</table>\n</div>\n"), isContainer:true, isAutoOrganized:true, isRightFixed:false, isLeftFixed:false, hasResizableColumns:true, nbZones:1, opacity:1, colWidths:[], minColWidth:20, minChildWidth:150, acceptTypes:[], mode:"right", allowAutoScroll:false, timeDisplayPopup:1500, isOffset:false, offsetDrag:{}, withHandles:false, handleClasses:[], _draggedWidget:null, _isResized:false, _activeGrip:null, _a11yOn:false, _canDisplayPopup:true, constructor:function (props, node) {
		...
		var children = dojo.query(".gridContainerZone > [widgetId]", this.containerNode).map(dijit.byNode);
		return children;
	}, onResized:function () {
		if (this.hasResizableColumns) {
			this._placeGrips();
		...
		var nbz = this.nbZones, childs = this.getNewChildren(), nbs = childs.length, res = Math.floor(nbs / nbz), mod = nbs % nbz, i = 0;
		for (var z = 0; z < nbz; z++) {
			for (var r = 0; r < res; r++) {
	...
	}, _insertService:function (z, p, service, first) {
		if (service === undefined) {
			return;
		}
		var zone = this.getZones()[z];
		var kidsZone = zone.childNodes.length;
		if (p === undefined || p > kidsZone) {
			p = kidsZone;
		}
		...
		this._placeGrips();
		return service.id;
	}, addService:function (service, z, p) {
		return this.addChild(service, z, p);
	}, addChild:function (service, z, p) {
		...
		dojo.forEach(this.grid, function (zone) {
			if (zone.grip) {
				...
				dojo.style(grip, {left:size + "px", height:height + "px"});
			}
		}, this);
	}, _getZoneByIndex:function (n) {
		return this.grid[(n >= 0 && n < this.grid.length ? n : 0)];
	}, getIndexZone:function (zone) {
		for (var z = 0; z < this.grid.length; z++) {
		...
		if (this._canDisplayPopup) {
			...
			popup.innerHTML = "this widget type is not accepted to be moved!";
