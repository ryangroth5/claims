...
if (!dojo._hasResource["drails.common"]) {
	dojo._hasResource["drails.common"] = true;
	dojo.provide("drails.common");
	dojo.require("drails.monkey");
	dojo.require("dojo.fx");
	drails._insertionMap = {"top":"first", "bottom":"last", "before":"before", "after":"after"};
	drails._xhrMap = {"asynchronous":["sync", function (v) {
		return !v;
	}], "method":["method", function (v) {
		return v.toLowerCase();
	}], "insertion":["place", function (v) {
		return drails._insertionMap[v];
	}], "parameters":["content", function (v) {
		return dojo.isObject(v) ? v : dojo.queryToObject(v);
	}], "evalScripts":"evalScripts"};
	drails._xhrCallbackMap = {"onUninitialized":null, "onLoading":null, "onLoaded":null, "onInteractive":null, "onCreate":null, "onComplete":null, "onFailure":"error", "onSuccess":"load"};
	dojo.declare("drails._base", null, {transformCallbacks:function (url, xhrArgs) {
			...
			if (dojoCallback) {
				if (this[protoCallback]) {
					...
					if (xhrArgs[protoCallback]) {
						dojo.connect(this, protoCallback, xhrArgs[protoCallback]);
					}
				} else {
					if (xhrArgs[protoCallback]) {
	...
	dojo.declare("drails.Request", [drails._base], {_requestOnConstruction:true, _transformedArgs:null, _transformedMethod:null, constructor:function (url, xhrArgs) {
		...
		this._transformedArgs.load = function (data) {
			dojo.eval(data);
		};
		this._transformedArgs.error = function (data) {
			alert("error -" + data);
		};
		...
		var o = {"headers":{"Accept":"text/javascript, text/html, application/xml, text/xml, */*"}};
		return o;
	}});
	dojo.declare("drails.Updater", [drails.Request], {stripRegExp:new RegExp("<script[^>]*>([\\S\\s]*?)</script>", "img"), _requestOnConstruction:false, _successNode:null, _failureNode:null, _strippedContent:null, constructor:function (target, url, xhrArgs) {
		xhrArgs = xhrArgs || {};
		...
		if (target) {
			this.interpolateTargets(target);
		}
		this.xhr(url, xhrArgs);
	...
	dojo.declare("drails.PeriodicalExecuter", null, {constructor:function (callback, frequency) {
	...
	dojo.declare("drails.TimedObserver", null, {element:null, executer:null, callback:null, lastValue:null, constructor:function (element, frequency, callback) {
		this.callback = callback;
		...
		if (dojo.isString(this.lastValue) && dojo.isString(value) ? this.lastValue != value : String(this.lastValue) != String(value)) {
	...
	dojo.declare("drails.Form.Element.Observer", [drails.TimedObserver], {getValue:function () {
		return this.element.name + "=" + (dojo.fieldToObject(this.element) || "");
	}});
	dojo.declare("drails.Form.Observer", [drails.TimedObserver], {getValue:function () {
		return dojo.objectToQuery(dojo.formToObject(this.element));
	}});
	dojo.declare("drails.EventObserver", null, {element:null, lastValue:null, constructor:function (element, callback) {
		this.element = dojo.byId(element);
		this.callback = callback;
		...
		if (type == "") {
			throw new Error("Invalid type for element: " + element.constructor.toString() + ".\tDid you forget to specify an input type in your markup?");
		}
		var evtType;
		switch (element.type) {
		  case "checkbox":
	...
	dojo.declare("drails.Form.Element.EventObserver", [drails.EventObserver], {registerCallbacks:function (element) {
		this.registerListenerForField(element);
	}, getValue:function () {
		return this.element.name + "=" + (dojo.fieldToObject(this.element) || "");
	}});
	dojo.declare("drails.Form.EventObserver", [drails.EventObserver], {registerCallbacks:function (element) {
	...
	dojo.declare("drails.Sortable", null, {options:null, source:null, sourceNode:null, connects:null, constructor:function (element, options) {
		this.connects = [];
	...
	dojo.declare("drails.dnd.Source", null, {connects:null, source:null, options:null, initSource:function (node) {
		dojo.require("dojo.dnd.Source");
	...
	(function () {
		var m = {"onSourceOver":"onDndSourceOver", "onStart":"onDndStart", "onDrop":"onDndDrop", "onCancel":"onDndCancel"};
		var a = [];
		for (var p in m) {
			a.push(p);
		}
	...
	dojo.declare("drails.Draggable", [drails.dnd.Source], {element:null, sourceNode:null, constructor:function (element, options) {
		this.element = dojo.byId(element);
		this.connects = [];
	...
	dojo.declare("drails.Droppable", [drails.dnd.Source], {constructor:function (element, options) {
		this.initSource(dojo.byId(element));
		this.applyOptions(options);
	}});
	dojo.declare("drails.Toggler", [dojo.fx.Toggler], {_lastState:null, getNextState:function () {
	...
	(function () {
		var _f;
		dojo.declare("drails.Effect", null, {fx:null, fxOptions:null, constructor:function (element, effect, options) {
			var ctor = _f._fxCtorMap[effect];
			if (!ctor) {
				throw new Error("'" + effect + "' is not a valid drails effect");
			}
		...
		_f = drails.Effect;
		dojo.mixin(_f, {togglers:{}, _lastTogglerId:1, _fxCtorMap:{"fade_in":dojo.fadeIn, "fade_out":dojo.fadeOut, "wipe_in":dojo.fx.wipeIn, "wipe_out":dojo.fx.wipeOut, "slide_to":dojo.fx.slideTo}, _toggleFxOptionMap:{"appear":"fade_in", "slide":"slide_to", "blind":"wipe_in"}, findToggleFxCtor:function (effect) {
			var ctor = _f._fxCtorMap[_f._toggleFxOptionMap[effect]];
			if (!ctor) {
				throw new Error("'" + effect + "' is not a valid drails toggle effect");
			}
			return ctor;
